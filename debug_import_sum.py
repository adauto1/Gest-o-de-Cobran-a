...
try:
    ...
    print(f"Total Rows Scan: {len(rows)}")
    ...
    for i, row in enumerate(rows):
        ...
        for cand in candidates_date:
            # Range from import_html.py: 550 <= left <= 630
        ...
        for cand in candidates_amount:
            # Range from import_html.py: 630 <= left <= 700
        ...
        if valid_date and valid_amount:
            ...
            total_sum += amount
            count += 1
            # print(f"Row {i}: Amount {amount} (Date: {valid_date['text']})")
        else:
            # Check if this row SHOULD have been matched
            # E.g. has amount but slightly off position
            # Or has date but missing amount
            if candidates_amount or candidates_date:
                 line_content = " | ".join([f"{el['text']} (x={el['left']})" for el in row])
                 unmatched_rows.append(f"Row {i}: {line_content}")
    ...
    if len(unmatched_rows) > 0:
        print(f"\nPotential Missed Rows ({len(unmatched_rows)}):")
