...
try:
    ...
    for i, row in enumerate(rows):
        ...
        if valid_date and valid_amount:
            ...
            # FILTER: Exclude if name looks like date
            if re.match(r'^\d{2}/\d{2}/', name):
                print(f"Skipping Row {i} (Name looks like date: {name})")
                continue
            
            # Additional filter: Exclude if name matches "Total"
            if "Total" in name and "Valor" not in name: # "Valor Total" line might be skipped by date logic, but safe to add
                 pass
